...
inbufptr            rmb       2         current pointer in input buffer (on DP)
inbufend            rmb       2         end of valid data in input buffer (on DP)
Datbuf              rmb       16
Txtbuf              rmb       BUFSZ     text buffer to build output lines in
inbuf               rmb       256       input file read buffer
                    rmb       128
datsz               equ       .
...
adlop               lda       b,u       Get byte from 32 bit address value
                    ...
                    leax      1,x
                    stx       <HexPtr
                    lbsr      readi     Get next block of data from file (returns B=# of bytes read)
                    lbcs      eofck     If error reading next block, go handle (return from there)
...
readi               tst       <D.Mem    Are we dumping from memory instead of file?
                    bne       redad     Yes, set up for next block in memory module

                    ;         Check     available bytes in buffer
                    ldd       <inbufend
                    subd      <inbufptr
                    bgt       readi_have_bytes
